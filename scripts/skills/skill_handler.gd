...
func cast(targets : Array[LivingEntity]) -> void:
	skill.cast_effects(source)
	skill.cast(source, targets)

func cooldown_done() -> void:
	var targets : Array[LivingEntity] = request_targets(skill.max_target_nb)
	if skill.cast_requirements(source, targets):
		cast(targets)
